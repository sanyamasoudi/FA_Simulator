...
class NFA:
    ...
    def deserialize_json(json_str: str) -> 'NFA':
        ...
        for state_str in json_fa["states"]:
            for symbol in fa.alphabet:
                for to_state_str in json_fa[state_str][symbol]:
                    fa.add_transition(fa.get_state_by_id(state_str[2:]), fa.get_state_by_id(to_state_str[2:]),
                                      symbol)
    ...
    def serialize_to_json(self) -> str:
        ...
        for state in self.states:
            fa[f"q_{state.id}"] = {}
            for symbol in self.alphabet:
                for to_state in json_fa[state_str][symbol]:
                    fa[f"q_{state.id}"][symbol].append(
                        f"q_{state.transitions[symbol].id}")
    ...
    def union(machine1: 'NFA', machine2: 'NFA') -> 'NFA':
        ...
        newMachine.add_transition(newMachine.get_state_by_id(
            newInitState.id), newMachine.get_state_by_id(machine1.init_state.id), "λ")
        newMachine.add_transition(newMachine.get_state_by_id(
            newInitState.id), newMachine.get_state_by_id(machine2.init_state.id), "λ")
    ...
    def concat(machine1: 'NFA', machine2: 'NFA') -> 'NFA':
        ...
        for fState_machine1 in machine1.final_states:
            newMachine.add_transition(
                newMachine.get_state_by_id(fState_machine1.id), newMachine.get_state_by_id(machine2.init_state.id), "λ")
    ...
    def star(machine: 'NFA') -> 'NFA':
        for fState in machine.final_states:
            machine.add_transition(fState, machine.init_state, "λ")
            machine.add_transition(machine.init_state, fState, "λ")
        return machine
